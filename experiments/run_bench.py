...
SUPPORTED_FPGA = {"qft2", "qft4", "grover2", "bell2"}
ALL_PROGRAMS = [
...
def run_fpga_prog(prog: str, runs: int, dump_vcd: bool) -> Dict[str, Optional[float]]:
    result: Dict[str, Optional[float]] = {"fpga_cycles": None, "status": None}
...
def write_csv(rows: List[Dict[str, Optional[str]]], path: Path) -> None:
    ...
    fieldnames = ["timestamp", "git_sha", "host", "prog", "fpga_cycles", "cpu_ms", "status"]
